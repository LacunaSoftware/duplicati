        ...
        public void Fix()
        {
            ...
            using(var cmd = m_connection.CreateCommand())
            {
                ...
                using(var upcmd = m_connection.CreateCommand())
                {
                    ...
                    upcmd.ExecuteNonQuery(string.Format(@"UPDATE ""{0}"" SET ""Obfuscated"" = @param1 + LEN(""RealPath"") + @param2 + ""ID"" + (CASE WHEN substr(""RealPath"", LEN(""RealPath"")) = @param3 THEN @param4 ELSE @param5 END) ", tablename), Platform.IsClientPosix ? "/" : "X:\\", Util.DirectorySeparatorString, Util.DirectorySeparatorString, Util.DirectorySeparatorString, ".bin");
